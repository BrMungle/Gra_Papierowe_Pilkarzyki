...
class MyModel(tf.keras.Model):
  def __init__(self):
    ...
    self.d1 = layers.Dense(50, activation="sigmoid", name="dense_1")
    self.d2 = layers.Dense(40, activation="sigmoid", name="dense_2")
    self.d3 = layers.Dense(8, activation="tanh", name="dense_3")
...
class gracz_lewy:
    ...
    @tf.function    
    def aktualizuj_siec(self,x,y,pozycja):
        mnoznik = np.zeros((1,8),np.float32)
        mnoznik[0,pozycja] = 1
        with tf.GradientTape() as tape:
            predictions = tf.linalg.matmul(mnoznik,self.model(x),transpose_b=True)
    ...
    def pobierz_stan(self,tablica):
        ...
        return(np.append(tab,[poz_y,poz_x]).reshape((1,101)))
    ...
    def ruch(self,tablica,eps,gamma):
        ...
        if nagroda != 0:
            ...
            self.aktualizuj_siec(tf.convert_to_tensor(stan),
            ...
            self.aktualizuj_siec(tf.convert_to_tensor(self.stany[los]),
                                 tf.convert_to_tensor(y),
                                 self.ruchy[los])
    ...
    def finalna_nagroda(self,nagroda):
        ...
        self.aktualizuj_siec(tf.convert_to_tensor(ostatni_stan),
                             tf.convert_to_tensor(nagroda),
                             ostatni_ruch)  
